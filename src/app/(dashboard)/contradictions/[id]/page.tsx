...
export default function ContradictionDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "evidence" | "traceability" | "affected" | "lifecycle"
  >("evidence");
    ...
    return (
      <PageContainer>
        <Stack align="center" gap={4}>
          ...
          <p className="text-muted-foreground text-sm">{error || "Contradiction not found"}</p>
      ...
      <Stack gap={6}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <div
                className={cn(
                  ...
                  SEVERITY_COLORS[contradiction.severity] ?? SEVERITY_COLORS["NEEDS_REVIEW"],
                )}
              >
            ...
            <p className="text-muted-foreground text-sm">{contradiction.description}</p>
          </div>
        </div>
            ...
            [
              {
                ...
                label: "Evidence",
                count:
                  contradiction.supportingEvidence.length +
                  contradiction.conflictingEvidence.length,
              },
              {
                key: "traceability",
                label: "Traceability",
                count: contradiction.traceabilityChain.length,
              },
                ...
                {contradiction.traceabilityChain.map((rec, idx) => (
                  ...
                  >
                    <div className="flex flex-1 flex-col gap-0.5">
                      <span className="text-foreground text-sm font-medium">{rec.entityName}</span>
                      <span className="text-muted-foreground text-xs">
                        v{rec.entityVersion} · {rec.entityStatus}
                      </span>
                    </div>
                    <div className="flex flex-1 flex-col gap-0.5">
                      {rec.documentName ? (
                ...
                <Stack gap={0}>
                  {contradiction.affectedEntities.map((ent) => (
                    <Link
                      ...
                      className="hover:bg-surface-hover border-border flex items-center gap-3 border-b px-4 py-3 transition-colors last:border-b-0"
                    >
                      ...
                      <div className="flex min-w-0 flex-1 flex-col">
                        ...
                        <span className="text-muted-foreground text-xs">
                          {ent.entityType} · {ent.relationship}
                        </span>
                      </div>
              ...
              <Panel padding="none">
                <Stack gap={0}>
                  {contradiction.lifecycleLogs.map((log) => (
                    <div
                      key={log.id}
                      className="border-border hover:bg-surface-hover flex items-center gap-3 border-b px-4 py-3 transition-colors last:border-b-0"
                    >
    ...
    <div className="hover:bg-surface-hover border-border flex items-center gap-3 border-b px-4 py-3 transition-colors last:border-b-0">
      ...
      {ev.documentName && (
        <div className="hidden flex-col items-end sm:flex">
          <span className="text-muted-foreground text-xs">{ev.documentName}</span>
          {ev.page && <span className="text-muted-foreground text-xs">Page {ev.page}</span>}
        </div>
      )}
    </div>
  );
