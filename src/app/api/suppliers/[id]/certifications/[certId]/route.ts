...
export async function GET(
  ...
  try {
    ...
    const cert = await getCertification(certId, id, orgId);
...
export async function PUT(
  ...
  try {
    ...
    const cert = await updateCertification(certId, id, orgId, parsed.data);
    return NextResponse.json({ data: cert });
  } catch (error) {
    if (error instanceof AppError)
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const { id, certId } = await params;
    await deleteCertification(certId, id, orgId);
    return NextResponse.json({ data: { message: "Certification deleted" } });
  } catch (error) {
    if (error instanceof AppError)
      return NextResponse.json(
        { error: error.message, code: error.code },
