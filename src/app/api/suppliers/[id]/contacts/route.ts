...
import { getContacts, createContact, createContactSchema } from "@/server/suppliers";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const contacts = await getContacts(id, orgId);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const contact = await createContact(id, orgId, parsed.data);
