...
import { getFacilities, createFacility, createFacilitySchema } from "@/server/suppliers";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const facilities = await getFacilities(id, orgId);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const facility = await createFacility(id, orgId, parsed.data);
