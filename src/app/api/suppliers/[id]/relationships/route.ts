...
import { getRelationships, createRelationship, createRelationshipSchema } from "@/server/suppliers";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rels = await getRelationships(id, orgId);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rel = await createRelationship(id, orgId, parsed.data);
