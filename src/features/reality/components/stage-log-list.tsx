...
import { RealityStageStatusBadge } from "./assessment-status-badge";
...
export function StageLogList({ assessmentId }: { assessmentId: string }) {
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `/api/reality/assessments/${assessmentId}/logs?page=${page}&pageSize=50`,
        );
  ...
  if (logs.length === 0) {
    return (
      <EmptyState
        ...
        description="This assessment has not recorded any stage executions yet."
      />
    );
  }
        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id}>
              ...
              <TableCell>
                <RealityStageStatusBadge status={log.status} />
              </TableCell>
