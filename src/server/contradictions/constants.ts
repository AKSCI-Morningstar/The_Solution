...
export const CONTRADICTION_TYPE_DESCRIPTIONS: Record<string, string> = {
  REQUIREMENT_CONTRADICTION:
    "Two or more requirements cannot simultaneously be satisfied under the same engineering context.",
  SPECIFICATION_CONTRADICTION:
    "Conflicting specifications reference the same engineering object with incompatible parameters.",
  DOCUMENT_CONTRADICTION:
    "Document content contains internally inconsistent statements or contradicts another document.",
  EVIDENCE_CONTRADICTION:
    "Evidence nodes present conflicting claims about the same engineering fact.",
  MATERIAL_CONTRADICTION: "Material specifications conflict for the same component or assembly.",
  SUPPLIER_CONTRADICTION:
    "Multiple suppliers with incompatible capabilities are assigned to the same entity.",
  ...
  RELATIONSHIP_CONTRADICTION:
    "Relationship definitions conflict or create impossible dependency cycles.",
