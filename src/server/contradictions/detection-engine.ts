...
function toTraceability(node: EvidenceNode, path: string[]): ContradictionTraceabilityRecord {
  return {
...
function toAffected(node: EvidenceNode, relationship: string): AffectedEntity {
  return {
    entityId: node.entityId ?? node.id,
    entityName: node.label,
    entityType: node.entityType ?? node.type,
...
function makeContradiction(
  ...
  const entityIds = nodes.map((n) => n.entityId).filter((id): id is string => id !== undefined);
  const documentIds = nodes.map((n) => n.documentId).filter((id): id is string => id !== undefined);
...
function detectRequirementContradictions(
  ...
  for (const req of requirements) {
    const edges = graph.edges.filter(
      (e) =>
        e.sourceId === req.id && (e.relationType === "REFERENCES" || e.relationType === "SUPPORTS"),
    );
...
function detectMaterialContradictions(
  ...
  const results: ContradictionRecord[] = [];
  const materials = Array.from(graph.nodes.values()).filter((n) => n.entityType === "MATERIAL");

  const byComponent = new Map<string, EvidenceNode[]>();
  for (const mat of materials) {
    const edges = graph.edges.filter((e) => e.sourceId === mat.id && e.relationType === "SUPPORTS");
    for (const edge of edges) {
      const target = graph.nodes.get(edge.targetId);
      if (!target?.entityId) continue;
...
function detectSupplierContradictions(
  ...
  for (const sup of suppliers) {
    const edges = graph.edges.filter((e) => e.sourceId === sup.id && e.relationType === "SUPPORTS");
    for (const edge of edges) {
      const target = graph.nodes.get(edge.targetId);
      if (!target?.entityId) continue;
...
function detectInterfaceContradictions(
  ...
  const results: ContradictionRecord[] = [];
  const interfaces = Array.from(graph.nodes.values()).filter((n) => n.entityType === "INTERFACE");
...
function detectDocumentContradictions(
  ...
  for (const [, docList] of byEntity) {
    ...
    const versions = docList
      .map((d) => d.documentVersion)
