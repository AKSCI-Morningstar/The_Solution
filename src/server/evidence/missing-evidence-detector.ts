...
function checkMissingTests(
  ...
  const hasTests = graph.edges.some(
    (e) =>
      (e.targetId === graph.rootId || e.sourceId === graph.rootId) &&
      e.relationType === "VERIFIES",
  );
...
function checkMissingCertification(
  graph: EvidenceGraph,
  seen: Set<string>,
): MissingEvidence[] {
  const missing: MissingEvidence[] = [];
...
function checkMissingSpecification(
  graph: EvidenceGraph,
  seen: Set<string>,
): MissingEvidence[] {
  const missing: MissingEvidence[] = [];
...
function checkMissingTraceability(
  graph: EvidenceGraph,
  seen: Set<string>,
): MissingEvidence[] {
  const missing: MissingEvidence[] = [];
