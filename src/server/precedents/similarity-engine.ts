...
export function computeSimilarity(
  ...
  if (context.project && precedent.relatedProjects.length > 0) {
    const match = precedent.relatedProjects.some(
      (p) => p.toLowerCase().includes(context.project!.toLowerCase()),
    );
    if (match) {
  ...
  if (context.question) {
    ...
    if (tokens.length > 0) {
      ...
      const summaryMatch = precedent.summary ? tokenMatch(precedent.summary, tokens) : { score: 0, matched: [] };
      ...
      if (bestToken > 0) {
        ...
        const allMatched = [...new Set([...titleMatch.matched, ...summaryMatch.matched, ...questionMatch.matched])];
