...
describe("contradiction detection engine", () => {
  it("returns empty result for graph with no contradictions", () => {
    const nodes = [makeNode({ id: "a", label: "Entity A", entityType: "COMPONENT", status: "ACTIVE" })];
  ...
  it("detects requirement contradictions with conflicting statuses", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const req1 = makeNode({ id: "req-1", label: "Req A", entityType: "REQUIREMENT", type: "REQUIREMENT", status: "ACTIVE", entityId: "req-ent-1" });
    const req2 = makeNode({ id: "req-2", label: "Req B", entityType: "REQUIREMENT", type: "REQUIREMENT", status: "REJECTED", entityId: "req-ent-2" });
    ...
    const result = detectContradictions(graph, ORG_ID);
    const reqConflicts = result.contradictions.filter((c) => c.type === "REQUIREMENT_CONTRADICTION");
  ...
  it("detects specification contradictions with different statuses", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const spec1 = makeNode({ id: "spec-1", label: "Spec A", entityType: "SPECIFICATION", type: "SPECIFICATION", status: "ACTIVE", entityId: "spec-ent-1" });
    const spec2 = makeNode({ id: "spec-2", label: "Spec B", entityType: "SPECIFICATION", type: "SPECIFICATION", status: "DRAFT", entityId: "spec-ent-2" });
    const edges: EvidenceEdge[] = [
    ...
    const result = detectContradictions(graph, ORG_ID);
    const specConflicts = result.contradictions.filter((c) => c.type === "SPECIFICATION_CONTRADICTION");
  ...
  it("detects material contradictions for same component", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const mat1 = makeNode({ id: "mat-1", label: "Aluminum", entityType: "MATERIAL", entityId: "mat-ent-1" });
    const mat2 = makeNode({ id: "mat-2", label: "Steel", entityType: "MATERIAL", entityId: "mat-ent-2" });
    const edges: EvidenceEdge[] = [
  ...
  it("detects supplier contradictions", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const sup1 = makeNode({ id: "sup-1", label: "Supplier A", entityType: "SUPPLIER", entityId: "sup-ent-1" });
    const sup2 = makeNode({ id: "sup-2", label: "Supplier B", entityType: "SUPPLIER", entityId: "sup-ent-2" });
    const edges: EvidenceEdge[] = [
  ...
  it("detects version contradictions from SUPERSEDES edges", () => {
    const old = makeNode({ id: "old", label: "Old Spec", entityType: "SPECIFICATION", status: "SUPERSEDED", entityId: "ent-old" });
    const newer = makeNode({ id: "new", label: "New Spec", entityType: "SPECIFICATION", status: "ACTIVE", entityId: "ent-new" });
    const edges: EvidenceEdge[] = [
      { id: "e1", sourceId: "new", targetId: "old", relationType: "SUPERSEDES" },
    ];
    const graph = makeGraph([old, newer], edges, "old");
    const result = detectContradictions(graph, ORG_ID);
    const versionConflicts = result.contradictions.filter((c) => c.type === "VERSION_CONTRADICTION");
  ...
  it("detects lifecycle contradictions (active depends on superseded)", () => {
    const active = makeNode({ id: "active", label: "Active Entity", entityType: "COMPONENT", status: "ACTIVE", entityId: "ent-active" });
    const superseded = makeNode({ id: "super", label: "Old Entity", entityType: "COMPONENT", status: "SUPERSEDED", entityId: "ent-super" });
    const edges: EvidenceEdge[] = [
      { id: "e1", sourceId: "active", targetId: "super", relationType: "SUPPORTS" },
    ];
    const graph = makeGraph([active, superseded], edges, "active");
    const result = detectContradictions(graph, ORG_ID);
    const lifecycleConflicts = result.contradictions.filter((c) => c.type === "LIFECYCLE_CONTRADICTION");
  ...
  it("detects evidence contradictions from CONTRADICTS edges", () => {
    const a = makeNode({ id: "a", label: "Evidence A", entityType: "COMPONENT", status: "ACTIVE", entityId: "ent-a" });
    const b = makeNode({ id: "b", label: "Evidence B", entityType: "COMPONENT", status: "ACTIVE", entityId: "ent-b" });
    const edges: EvidenceEdge[] = [
      { id: "e1", sourceId: "a", targetId: "b", relationType: "CONTRADICTS" },
    ];
    const graph = makeGraph([a, b], edges, "a");
    const result = detectContradictions(graph, ORG_ID);
    const evidenceConflicts = result.contradictions.filter((c) => c.type === "EVIDENCE_CONTRADICTION");
  ...
  it("detects certification contradictions", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const cert1 = makeNode({ id: "cert-1", label: "Cert A", entityType: "CERTIFICATION", type: "CERTIFICATION", status: "ACTIVE", entityId: "cert-ent-1" });
    const cert2 = makeNode({ id: "cert-2", label: "Cert B", entityType: "CERTIFICATION", type: "CERTIFICATION", status: "EXPIRED", entityId: "cert-ent-2" });
    const edges: EvidenceEdge[] = [
    ...
    const result = detectContradictions(graph, ORG_ID);
    const certConflicts = result.contradictions.filter((c) => c.type === "CERTIFICATION_CONTRADICTION");
    expect(certConflicts).toHaveLength(1);
  });

  it("detects document contradictions with different versions", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const doc1 = makeNode({ id: "doc-1", label: "Spec Doc", type: "EXTRACTED_FACT", entityId: "ent-1", documentId: "d1", documentName: "spec.pdf", documentVersion: 1 });
    const doc2 = makeNode({ id: "doc-2", label: "Spec Doc v2", type: "EXTRACTED_FACT", entityId: "ent-1", documentId: "d2", documentName: "spec.pdf", documentVersion: 2 });
    const edges: EvidenceEdge[] = [
  ...
  it("detects duplicate entities", () => {
    ...
    const graph = makeGraph(nodes, [], "a");
    const result = detectContradictions(graph, ORG_ID);
    const dupConflicts = result.contradictions.filter((c) =>
      c.label.includes("Duplicate"),
    );
  ...
  it("detects stale evidence", () => {
    ...
    const graph = makeGraph(nodes, [], "a");
    const result = detectContradictions(graph, ORG_ID);
    const staleConflicts = result.contradictions.filter((c) =>
      c.label.includes("Stale"),
    );
  ...
  it("detects broken references in edges", () => {
    ...
    const result = detectContradictions(graph, ORG_ID);
    const brokenConflicts = result.contradictions.filter((c) =>
      c.label.includes("Broken"),
    );
  ...
  it("detects interface contradictions with different statuses", () => {
    const target = makeNode({ id: "target", label: "Component", entityType: "COMPONENT", entityId: "ent-1", status: "ACTIVE" });
    const iface1 = makeNode({ id: "iface-1", label: "Interface A", entityType: "INTERFACE", status: "ACTIVE", entityId: "iface-ent" });
    const iface2 = makeNode({ id: "iface-2", label: "Interface B", entityType: "INTERFACE", status: "DRAFT", entityId: "iface-ent" });
    const edges: EvidenceEdge[] = [
    ...
    const result = detectContradictions(graph, ORG_ID);
    const ifaceConflicts = result.contradictions.filter((c) => c.type === "INTERFACE_CONTRADICTION");
